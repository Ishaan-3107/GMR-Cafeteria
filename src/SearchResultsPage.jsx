...
// Completely isolated modal component (same as CafeteriaMenu)
...
export default function SearchResultsPage() {
  ...
  // Get current quantity for modal
  ...
  useEffect(() => {
    const fetchAllData = async () => {
      setLoading(true);
      try {
        const menuRes = await fetch("http://localhost:5000/api/all-menu-items");
        const menuData = await menuRes.json();
        setAllFoodItems(menuData);
        ...
        console.error("Error fetching universal menu data:", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  const filteredItems = useMemo(() => {
    ...
    return allFoodItems.filter((item) => {
      const vendor = allVendors.find((v) => v.id === item.vendor_id);
      return (
        item.item_name.toLowerCase().includes(lowerCaseSearchTerm) ||
        (vendor && vendor.name.toLowerCase().includes(lowerCaseSearchTerm)) ||
        (item.category &&
          item.category.toLowerCase().includes(lowerCaseSearchTerm))
      );
    });
  }, [allFoodItems, allVendors, searchTerm]);
      ...
      {filteredItems.length === 0 ? (
        <Box textAlign="center">
          ...
          <Typography variant="body1" sx={{ mt: 2 }}>
            No food items or vendors found matching "{searchTerm}".
          </Typography>
        </Box>
      ) : (
        <Box sx={{ width: "100%" }}>
